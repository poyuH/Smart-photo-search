        ...
        try:
            ...
            for index, link in enumerate(links):
                if limit is not None and current + index >= limit:
                    return
                t = threading.Thread(
                    target=download, args=(pool_sema, link, output_dir)
                )
                t.start()
                current += 1
            last = links[-1]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-s", "--search-string", help="Keyword to search", required=False
    )
    parser.add_argument("-o", "--output", help="Output directory", required=False)
    parser.add_argument(
        "--limit",
        help="Make sure not to search for more than specified amount of images.",
        required=False,
        type=int,
    )
    ...
    if args.search_string:
        fetch_images_from_keyword(
            pool_sema, args.search_string, output_dir, args.limit
        )
        ...
        for keyword in inputFile.readlines():
            output_sub_dir = os.path.join(
                output_dir_origin, keyword.strip().replace(" ", "_")
            )
            if not os.path.exists(output_sub_dir):
                os.makedirs(output_sub_dir)
            fetch_images_from_keyword(
                pool_sema, keyword, output_sub_dir, args.limit
            )
            backup_history()
            time.sleep(10)
        inputFile.close()
